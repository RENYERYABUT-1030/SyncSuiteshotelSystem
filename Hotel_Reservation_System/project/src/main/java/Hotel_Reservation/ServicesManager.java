...
public class ServicesManager {
    ...
    // For production, set a real key in Settings > SMS Settings.
    // FIX: This used to be a hardcoded constant with no way to change it
    // without editing source code. Now it's read from system_settings
    // (the same table SettingsPanel writes to), falling back to the free
    // "textbelt" test key if nothing has been configured yet.
    private static String getSmsApiKey() {
        try (Connection conn = EnhancedDBConnection.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                "SELECT setting_value FROM system_settings WHERE setting_key = 'sms_api_key'");
            ...
            System.err.println("Could not load sms_api_key setting, using default: " + e.getMessage());
        }
        return "textbelt";
    }
        ...
        new Thread(() -> {
            try {
                ...
                String postData = "phone=" + URLEncoder.encode(normalizedPhone, "UTF-8")
                                ...
                                + "&key=" + URLEncoder.encode(getSmsApiKey(), "UTF-8");
