...
public class EnhancedDashboardFrame extends JFrame {
    ...
    // NEW: CardLayout container so sidebar/quick-action buttons can actually
    // switch which panel is shown, instead of doing nothing when clicked.
    private CardLayout cardLayout;
    private JPanel cardsPanel;
    private static final String CARD_DASHBOARD = "DASHBOARD_HOME";
    ...
    private void initialize() {
        ...
        // NEW: cardsPanel holds every "page" (dashboard home + the actual
        // feature panels). Only one is visible at a time; cardLayout.show()
        // switches between them when a nav/quick-action button is clicked.
        cardLayout = new CardLayout();
        cardsPanel = new JPanel(cardLayout);
        cardsPanel.setOpaque(false);
        cardsPanel.add(contentPanel, CARD_DASHBOARD);
    ...
    /**
     * NEW: Central navigation handler. Adds the target panel to cardsPanel
     * the first time it's requested (lazy init), then shows it.
     * This is what was completely missing before - nothing called this,
     * so every button just sat there doing nothing on click.
     */
    private void navigateTo(String target) {
        ...
        try {
            switch (target) {
                case CARD_DASHBOARD:
                    cardLayout.show(cardsPanel, CARD_DASHBOARD);
                    return;
                case "NewBooking":
                    panel = new NewBookingPanel();
                    break;
                case "RoomAvailability":
                    panel = new RoomAvailabilityPanel();
                    break;
                case "ManageBookings":
                    panel = new ManageBookingPanel();
                    break;
                case "Maintenance":
            ...
            // FIX: previously, an exception while building a panel (e.g. a
            // DB error in its constructor) would propagate up silently and
            // leave the screen blank with nothing telling you why. Now it
            // shows the actual error instead of a mystery blank page.
            ex.printStackTrace();
        ...
        // Re-fetching a panel each click gives fresh data (e.g. refreshed
        // room list). Remove any previous instance under this key first so
        // cardsPanel doesn't accumulate stale duplicates.
    ...
    private JPanel createSidebar() {
        ...
        // Maps each visible label to the internal navigation key used by navigateTo()
        String[] navKeys = {
            CARD_DASHBOARD, "NewBooking", "RoomAvailability", "ManageBookings",
            "Maintenance", "Reports", "AuditLog", "Settings"
        };

        for (int i = 0; i < navItems.length; i++) {
            JButton btn = createNavButton(navItems[i]);
            String target = navKeys[i];
            btn.addActionListener(e -> navigateTo(target)); // NEW: this was missing entirely
            sidebar.add(btn);
    ...
    private JPanel createQuickActionsPanel() {
        ...
        String[][] actions = {
            {"🆕 New Booking", "Create a new guest booking", "NewBooking"},
            {"📋 View Bookings", "Manage existing bookings", "ManageBookings"},
            {"🔧 Maintenance", "Room maintenance tasks", "Maintenance"},
            {"💰 Process Payment", "Handle guest payments", "ManageBookings"},
            {"👥 Guest Services", "Request special services", "ManageBookings"},
            {"📊 View Analytics", "Business analytics & reports", "Reports"}
        };
    ...
    private JPanel createActionButton(String title, String description, String target) {
        ...
        button.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                navigateTo(target); // NEW: this was missing entirely - cards were unclickable
            }
        });
